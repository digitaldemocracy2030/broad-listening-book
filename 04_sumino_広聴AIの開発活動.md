...
TTTCではembeddingの元次元（高次元）でクラスタリングを実行していたが、広聴AIでは次元削減して2次元に圧縮してからクラスタリングを実行する。TTTCでは、同一クラスタに属する意見が散布図上で飛び地になってしまい、ユーザーの解釈が難しくなるという課題があった。2次元圧縮後にクラスタリングすることで、この問題を解消した。

ただし、このアプローチにも一長一短がある。クラスタリングの精度が下がり、関連性の薄い意見が同じクラスタにまとめられやすくなるというトレードオフが存在する。
