...
### From Talk to the City to Kouchou AI
![Technical lineage of Kouchou AI](images/10_kouchouai_technical_genealogy.png)

The origins of Kouchou AI trace back to “Talk to the City (TTTC),” an open-source tool for collecting and visualizing large volumes of opinions developed by the U.S.-based AI Objective Institute. TTTC has two variations: “TTTC Scatter,” which classifies and visualizes opinions using contextual vectors and scatter plots, and “TTTC Turbo,” which uses direct classification by an LLM to present results in a list-view format. Both have been used in practice in places such as Taiwan and elsewhere around the world.
...
The biggest feature of Kouchou AI is that it transformed TTTC Scatter from a command-line tool into a web application that can be operated through a browser. Hosting the application still requires engineering skills, but once it is up and running, analyses can be performed without development knowledge, and anyone can access the results simply by sharing a URL.

The main differences between Kouchou AI and TTTC Scatter are as follows.

| Perspective | TTTC Scatter | Kouchou AI |
|------|------|--------|
...
Kouchou AI also introduced several improvements to the analysis algorithm.

**Hierarchical clustering**

TTTC Scatter performed clustering at a single level, but Kouchou AI performs clustering hierarchically. This makes it possible to understand the structure of opinions at multiple levels of granularity. In addition to the conventional scatter-plot visualization, a treemap was also added to visualize the hierarchical structure.
...
TTTC Scatter and Kouchou AI use different clustering algorithms.
...
May 2025 became a turning point for the development of Kouchou AI—a moment when “deceleration and acceleration happened at the same time.”
...
In OSS development, when work continues with only a small number of people, there are times when the back-and-forth of review and decision-making thins out and progress becomes difficult. In Kouchou AI as well, that phase surfaced after the community dispersed.

After the turning point in May 2025, development of Kouchou AI remained in a difficult phase for some time. The density of day-to-day communication declined, and the exchanges needed for PR review and decision-making increasingly stalled. As reviews became harder to get, changes were less likely to be merged and more likely to sit idle.
